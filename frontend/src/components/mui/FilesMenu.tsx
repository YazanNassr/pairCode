...
export default function LongMenu() {
    ...
    const options = editorState.project.files.map(file => `${file.parentPath}/${file.fileName}`)
    ...
    const handleChoose = (option: string) => {
        setEditorState(state => ({
            ...state,
            activeFile: state.project.files.find(file => `${file.parentPath}/${file.fileName}` === option) || state.activeFile
        }));
    };
